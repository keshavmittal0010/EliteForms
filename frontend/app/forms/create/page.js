...
export default function CreateFormPage() {
  ...
  const handleSaveForm = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to save form. Try again.');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="flex-1 pl-0 lg:pl-60 flex flex-col min-h-screen">
        ...
        <div className="sticky top-16 z-30 flex items-center justify-between border-b border-zinc-900 bg-[#030303]/80 backdrop-blur-md px-6 py-3.5">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-sm font-bold text-white uppercase tracking-wider">Builder</span>
          </div>
          ...
          <div className="flex items-center space-x-3.5">
            ...
            <button
              ...
              className="inline-flex items-center space-x-2 rounded-xl border border-zinc-800 bg-zinc-900/30 px-3.5 py-2 text-xs font-semibold text-zinc-300 hover:text-zinc-100 hover:border-zinc-700 transition-all"
            >
              {isPreview ? (
                <>
        ...
        <main className="flex-1 p-6 md:p-8 max-w-6xl w-full mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <div className={`premium-panel rounded-2xl p-5 space-y-3.5 shadow-md ${isPreview ? 'opacity-40 pointer-events-none' : ''}`}>
            ...
            <div className="premium-panel rounded-2xl p-5 space-y-2 text-[10px] text-zinc-500 leading-relaxed">
              ...
              <p>Option-based components (Dropdowns, Radios, Checkboxes) require at least one choice to validate successfully.</p>
            </div>
          </div>
